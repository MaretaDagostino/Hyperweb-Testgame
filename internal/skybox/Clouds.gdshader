...
// and https://www.shadertoy.com/view/4dsXWn for 2d clouds
uniform vec3 WIND; // wind direction vector multiplied with wind force
...
{
	// just an example of randomness in shaders from internet
...
{
	lowp float f = 0.0;
	f += 0.50000 * noise(0.5 * (p + vec3(0.0, 0.0, -time * 0.275)));
	f += 0.25000 * noise(1.0 * (p + vec3(0.0, 0.0, -time * 0.275)));
	f += 0.12500 * noise(2.0 * (p + vec3(0.0, 0.0, -time * 0.275)));
	f += 0.06250 * noise(4.0 * (p + vec3(0.0, 0.0, -time * 0.275)));
	return  f * mix(c1, c2, p * 0.5 + 0.5);
}

lowp float MapSH(lowp vec3 p, lowp float cloudy, lowp vec3 offset, lowp float CLOUD_UPPER)
{
	lowp float h = -(get_noise(p * 0.0003 + offset, 2.76434) - cloudy - 0.6);
	h *= smoothstep((HEIGHT + 0.1) * CLOUD_UPPER + 100.0, (HEIGHT + 0.1) * CLOUD_UPPER, p.y);
	return h;
}
...
lowp vec4 clouds_2d(lowp vec3 rd, lowp vec3 wind)
{
	...
	for (int i = 0; i < min(STEPS, 5); i++)
	{
		...
		shade.y = max(h, 0.0); 
        shade.x = clamp(-(h - MapSH(p + SUN_POS * 200.0, cloudy, wind, CLOUD_UPPER)) *2.0, 0.05, 1.0);
		shadeSum += shade * (1.0 - shadeSum.y);
		p += add;
	}
	lowp vec3 clouds = mix(vec3(pow(shadeSum.x, 0.6)), vec3(1.0), (1.0 - shadeSum.y) * 0.4);
...
void fragment(){
	...
	rd.x *= -1.0;
	// This is the vector of displacement of the sphere relative to zero coordinates.
	// Here you can set the height of the clouds. That is, to make a sphere with clouds higher or lower.
	lowp vec3 ro = vec3(0.0, -200.0 * HEIGHT + 40.0, 0.0);
	lowp vec4 cld = vec4(0.0);
	lowp float skyPow = dot(rd, vec3(0.0, -1.0, 0.0));
	lowp float horizonPow =1.0 - pow(1.0 - abs(skyPow), 5.0);
	if(rd.y > 0.0)
	{
		if (STEPS < 20) cld = clouds_2d(rd, WIND * TIME);
		else cld = clouds_3d(ro, rd, WIND * TIME / SIZE);
		cld = clamp(cld, 0.0, 1.0);
		cld.rgb += 0.04 * cld.rgb * horizonPow;
		// Here we dissolve the clouds in the horizon for a smooth transition to the horizon line.
		cld *= clamp((1.0 - exp(-2.3 * pow(max(0.0, horizonPow), 2.6))), 0.0, 1.0);
	}
	else
	{
		cld.rgb = cube_bot(rd, vec3(1.5, 1.49, 1.71), vec3(1.1, 1.15, 1.5), TIME);
		cld.a = 1.0;
		cld *= clamp((1.0 - exp(-1.3 * pow(max(0.0, horizonPow), 2.6))), 0.0, 1.0);
	}
