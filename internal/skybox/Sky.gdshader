...
uniform vec3 LIGHTNING_STRENGTH;
uniform vec3 LIGHTNING_POS; // normalize this vector in script!
...
{
	lowp float ca = cos(angle);
	lowp float sa = sin(angle);
	return v * mat3(
		vec3(ca, 0.0, -sa),
		vec3(0.0,1.0, 0.0),
		vec3(sa, 0.0, ca));
...
{
	//just random function. Used for stars.
	return fract(sin(dot(co.xy, vec2(12.9898, 78.233))) * 43758.5453);
}

//someone else's code, there are problems with the movement of the moon on y-axis
lowp vec2 uv_sphere(lowp vec3 rd, lowp vec3 pos, lowp float scale)
{
...
lowp float draw_moon(lowp vec3 rd) 
{
	...
	return texture(MOON, uv).r * alpha;
}
...
lowp vec3 draw_night_sky (lowp float sky_amount, lowp vec3 rd, lowp float cld_alpha, lowp float time)
{
	...
	// If the light from the Sun does not obscure the stars at sunrise/sunset
	// and does not cover the clouds and moon
	if (sky_amount < 0.01 && cld_alpha == 0.0 && moon_amount < 0.01)
	{
		// The higher the stars, the fewer they are. Since the spherical panorama 
		// does not allow uniform coverage, the pixel density at height is higher.
		if (rand(rd.zx) - rd.y * 0.0033 > 0.996)
		{
	...
	// attenuation of the brightness of the moon (for sunrise and sunset)
	moon_amount *= 1.0 - attenuation;
	// here we cast a shadow on the moon as moon phase
	moon_amount = clamp(mix(0.0, moon_amount, smoothstep(0.9, 1.0, (1.0 - moon_radius) * 0.5
			+ length(MOON_POS - rd + MOON_PHASE) * (1.0 - moon_radius * 0.75))), 0.001, 1.0);
	// here we mix with the clouds so that there is no black border,
	// but so that the moon does not shine through the clouds.
	night_sky += moon_tint.rgb * moon_amount * (clamp(1.0 - cld_alpha - 0.2, 0.0, 1.0));
	return night_sky;
}

lowp vec3 getSkyAbsorption(lowp vec3 color, lowp float h)
{
	return exp2(color * -h) * 2.0;
}

lowp float horizon_limiter (lowp float h)
{
	// eliminate the dark line and other artefacts on the horizon with clamp
	return clamp(abs(h), 0.1 + smoothstep(0.0, 0.3, SUN_POS.y) * 0.2, 1.0);
}

lowp float zenithDensity(lowp float dens)
{
	return sky_density / pow(max(dens, 0.0), 0.75);
}

lowp float getSunPoint(lowp vec3 p, lowp vec3 lp)
{
	return min(pow(max(dot(p, lp), 0.0), 100.0 / sun_radius) * 100.0, 2.0);
}

lowp float getRayleigMultiplier(lowp vec3 p, lowp vec3 lp)
{
	return sky_rayleig_coeff + pow(1.0 - clamp(distance(p, lp), 0.0, 1.0), 2.0) * 3.14159265359 * 0.5;
}
...
lowp vec3 getAtmosphericScattering(lowp vec3 p, lowp vec3 lp)
{
	...
	sky = mix(sky * absorption, sky / (sky + 0.5), sunPointDistMult) + getSunPoint(p, lp)
			* absorption + getMie(p, lp) * sunAbsorption;
...
lowp vec3 get_gradient_sky(lowp vec3 rd,lowp vec3 sun)
{
	...
	if (sun.y > 0.0) 
	{
		sky_color = texture(sky_gradient_texture, vec2(sun.y * (1.0 - size_gradient_window)
				+ rd.y * size_gradient_window, 0.5)).rgb;
	}
	else
	{
	...
	sky_color = mix(sky_color, vec3(0.0), smoothstep(1.0, 2.5, distance(rd, sun)));
	lowp vec3 sun_color = vec3(1.0);
	sky_color += sun_color * min(pow(max(dot(rd, SUN_POS), 0.0), 40.0 / sun_radius) * 5.0, 1.0)
			+ sun_color * min(pow(max(dot(rd, SUN_POS), 0.0), 10.0) * 0.3, 1.0);
	return sky_color;
}
...
void fragment(){
    ...
    uv.x = 2.0 * uv.x - 1.0;
    uv.y = 2.0 * uv.y - 1.0;
	// transform UV to spherical panorama 3d coords
	lowp vec3 rd = normalize(rotate_y(rotate_x(vec3(0.0, 0.0, 1.0),
			-uv.y * 3.1415926535 / 2.0), -uv.x * 3.1415926535));
	// the x-axis is inverted on the godot scene for unknown reasons
	rd.x *= -1.0;
	
	// the axis is inverted on the godot scene for unknown reasons
	lowp vec4 cld = texture(cloud_env_texture, 1.0 - UV);
	// lighten the clouds depending on the height of the Sun, calculated in the script
	cld.rgb *= attenuation;
	cld *= clouds_tint;
	lowp vec3 sky;
	...
	lowp float lightning = texture(lightning_texture,
					uv_sphere(rd,LIGHTNING_POS,0.3)).r * LIGHTNING_STRENGTH.r;
	// flash of light in the sky simulates a lightning strike
	sky = mix (sky.rgb, LIGHTNING_STRENGTH, 0.5) + lightning;
