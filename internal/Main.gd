...
func _ready():
	...
	if Globals.is_server == true:
		...
		# FIXME: Fallback if somebody's Level.tscn has no spawn point
...
func _on_connection_failed():
	# FIXME: Better reaction and information to user, endless grey screen here
	get_tree().set_multiplayer_peer(null)

# Client: Quit if network breaks
func _on_server_disconnected():
	# FIXME: Send information to user instead of silent crash 
	get_tree().quit()
...
func _on_connect_pressed():
	...
	# FIXME: send information to user that the game is waiting for
